...
WARNING: This library is not suitable for serving assets from Universal APIs, Admin Extensions, or ID Providers. For these use cases, use link:https://developer.enonic.com/docs/lib-static/stable[lib-static] instead.
...
[NOTE]
====
When enabled, every `assetUrl()` call that receives the page `response` object will append a `Link` response header entry (`rel=preload`).
====
+
<5> Enable generation of `Link` preload response header values from `assetUrl()` calls.
+
[CAUTION]
====
...
The Asset library bundles an Asset API endpoint that handles the serving of assets. An Asset API URL may look something like this: `/_/{app-name}:asset/{app-fingerprint}/path/to/asset.ext`

If Cache Busting is disabled (cacheBust: false), the URL will look like this `/_/{app-name}:asset/path/to/asset.ext`

NOTE: In case of a mismatched fingerprint, the API will still respond with the asset contents and 200 code. But with a Cache-Control: `private, no-store` header. This prevents cache-pollution and makes website rendering more resilient to app redeployments especially in development when app fingerprint changes rapidly.

=== API Descriptor

The Asset API is implemented as a Universal API endpoint and is configured via a descriptor file at `src/main/resources/apis/asset/asset.yaml`:
...
The `allow` field controls access to the API endpoint. The Asset API uses `role:system.everyone` to enable public access, which is required for serving static assets like CSS, JavaScript, and images to all users.

IMPORTANT: To use the Asset API, you must explicitly list it in your Site, WebApp, or AdminTool descriptor's `apis:` field. Universal APIs are not exposed by default for security reasons.
...
For more information about Universal APIs and security configuration, see link:https://developer.enonic.com/docs/xp/stable/api[Enonic XP Universal API documentation].

=== Etag

The API calculates and caches ETag for each requested asset. Cache is cleared on application restart. ETag is used to respond with `304 Not Modified` for conditional requests.
...
! as          ! string ! <optional> !         ! Resource type for preload Link header (`style`, `script`, `image`, etc)
! response    ! object ! <optional> !         ! Response object. When `http2PushPreload=true`, appends this URL to the `link` response header as `rel=preload`
!===
