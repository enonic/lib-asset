...
Scenario: Backwards compatibility: Should NOT generate trailing slash when path is ''
  Given there is no configuration file
  When I call assetUrl with the following parameters:
    | parameter | value |
    | path      |       |
  Then I should get the following url "/webapp/com.example.myproject/_/service/com.example.myproject/asset/1234567890123456"

Scenario: Returns service "root" when called without parameters
  Given there is no configuration file
  When I call assetUrl with the following parameters:
    | parameter | value |
  Then I should get the following url "/webapp/com.example.myproject/_/service/com.example.myproject/asset/1234567890123456"

Scenario: Returns service "root" with trailing slash when path is '/'
  Given there is no configuration file
  When I call assetUrl with the following parameters:
    | parameter | value |
    | path      | /     |
